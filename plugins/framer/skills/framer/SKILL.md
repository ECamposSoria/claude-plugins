...
description: >
  ...
  **Mandatory precondition**: run `npx @framer/agent@latest setup` and let it complete **BEFORE** doing any Framer work.
allowed-tools: ["Bash(npx @framer/agent:*)", "Bash(npx @framer/agent@latest:*)"]
---

> **Packaging note (eze-claude-plugins):** Framer does **not** ship an MCP server — its
> official integration is Skills + the `@framer/agent` CLI. This plugin bundles the
> entry skill. The first command below (`setup`) installs the full official skill suite
> (code-components + the dynamically generated project skill) into `~/.claude/skills`,
> properly templated. Requires Node ≥22 (Framer recommends ≥24). The CLI is fetched on
> demand via `npx`; no global install is needed.
