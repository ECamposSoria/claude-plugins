---
name: adloop
description: Use whenever the user asks about Google Ads or GA4 — running campaigns, analyzing performance, diagnosing conversion drops, creating ads or campaigns, managing keywords or negative keywords, budget planning, tracking validation, landing page analysis, attribution, or any AdLoop MCP tool (health_check, run_ga4_report, get_campaign_performance, draft_campaign, draft_responsive_search_ad, analyze_campaign_conversions, discover_keywords, estimate_budget, confirm_and_apply, etc). Loads orchestration rules, GAQL reference, and safety protocols (dry-run, budget caps, Broad Match + Manual CPC protection, GDPR consent awareness).
---
